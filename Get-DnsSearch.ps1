...
param(
    ...
    process{
        ...
        ForEach ($Zone in $Zones) {
            ...
            Get-DnsServerResourceRecord -Computername $ServerName -ZoneName $Zone | ? { ($_.RecordData.IPv4Address -eq "$DNSLookup") -OR
                                                                                        ($_.RecordData.HostNameAlias -eq "$DNSLookup") -OR 
                                                                                        ($_.HostName -eq "$DNSLookup") }
